...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/inked';
...
const CATEGORY_MAP = {
    'technology': ['Hacker News', 'TechCrunch', 'Arstechnica', 'digitaltrends', 'Polygon'],
    'space': ['SpaceNews', 'NASA', 'ESA', 'NASASpaceflight', 'Spaceflight Now', 'European Spaceflight', 'SpacePolicyOnline.com', 'Space Scout'],
    'business': ['americanbankingnews', 'Business Line', 'CNBC', 'Seeking Alpha', 'actionforex', 'mainstreet'],
    'entertainment': ['dailymail', 'Metro', 'Mail', 'Entertainment'],
    'world': ['The Guardian', 'ke', 'The New York Times', 'WorldNews', 'BBC', 'BBC News - Middle East', 'latimes'],
    'sports': ['Sporting News', 'Dk Pittsburgh Sports News', 'The Independent - Sports', 'Yahoo Sports', 'Complete Sports Nigeria', 'Essentially Sports'],
...
app.get('/api/trending', async (req, res) => {
    try {
        ...
        const INDIAN_SOURCES = [
            ...
            "NDTV", "India Today", "Financial Express", "Economic Times", "Google News India"
        ];
...
app.post('/api/orchestrate', async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'Flow completed successfully', data: scrapeData });
    } catch (error) {
...
app.post('/api/takedown', async (req, res) => {
    try {
        ...
        const doc = await Takedown.create({ name, email, organisation, url, reason, description });
        console.log(`📋 Takedown request received from ${email} for ${url}`);
        res.json({ success: true, reference: `TDN-${doc._id.toString().slice(-6).toUpperCase()}` });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});
...
// Start Server
app.listen(PORT, () => {
    console.log(`🚀 Main Backend Serving API is running on http://localhost:${PORT}`);
});
