...
const formatTime = (_, timeStr) => {
  if (timeStr) {
    ...
    if (p.length >= 2) return p[0] + 'h ago';
  }
  return '2h ago';
};
...
const TrendingCard = ({ item, onPress }) => (
  ...
  >
    {item.image_link ? (
      <Image source={{ uri: item.image_link }} style={styles.trendImage} />
    ) : (
      <View style={[styles.trendImage, { backgroundColor: '#CCC' }]} />
    )}
    <LinearGradient
    ...
    <View style={styles.trendContent}>
      ...
      <Text style={styles.trendHeadline} numberOfLines={2}>
        {item.headline}
      </Text>
    </View>
  </TouchableOpacity>
...
const ArticleCard = ({ item, onPress, onOpenComments, isDark, navigation }) => {
  const {
    user,
    toggleLike,
    toggleBookmark,
  ...
  const [isLiked, setIsLiked] = useState(
    () =>
      checkIsLiked(item.headline) ||
      (user?.likedArticles || []).includes(item.headline),
  );
  const [likes, setLikes] = useState(item.likes || 0);

  const [isSaved, setIsSaved] = useState(
    () =>
      checkIsBookmarked(item) ||
      (user?.savedArticles || []).some(a => a._id === item._id),
  );

  const [commentsCount, setCommentsCount] = useState(
    (item.comments || []).length,
  );
  const [saves, setSaves] = useState(item.saveCount || 0);

  useEffect(() => {
    const sub = DeviceEventEmitter.addListener('ArticleUpdated', data => {
      if (data.articleId === item._id || data.articleId === item.headline) {
        if (data.type === 'like') {
          setLikes(prev => Math.max(0, prev + data.delta));
          setIsLiked(data.delta > 0);
        } else if (data.type === 'save') {
          setSaves(prev => Math.max(0, prev + data.delta));
          setIsSaved(data.delta > 0);
    ...
    return () => sub.remove();
  }, [item._id, item.headline]);

  // Sync with incoming data (e.g., on pull-to-refresh)
  useEffect(() => {
    setLikes(prev => Math.max(prev, item.likes || 0));
    setSaves(prev => Math.max(prev, item.saveCount || 0));
    setCommentsCount(prev => Math.max(prev, (item.comments || []).length));
  }, [item.likes, item.saveCount, item.comments]);
  ...
  const handleCardPress = () => {
    const updatedArticle = {
      ...
      saveCount: saves,
      comments:
        item.comments?.length >= commentsCount
          ? item.comments
          : new Array(commentsCount).fill({}),
    };
    if (onPress) {
      onPress(updatedArticle);
    } else if (navigation) {
      navigation.navigate('ArticleDetail', { article: updatedArticle });
    }
  };
  ...
  const handleLike = async () => {
    if (!user) return navigation?.navigate('AuthModal', { pendingAction: { type: 'like', article: item } });
    toggleLike(item);
  };

  const handleSave = async () => {
    if (!user) return navigation?.navigate('AuthModal', { pendingAction: { type: 'save', article: item } });
    toggleBookmark(item);
  };
    ...
    try {
      await NativeShare.share({
        message: `${item.headline}\n\nRead more on NewsOnTip: ${
          item.link || 'https://newsontip.online'
        }`,
      });
      await safeFetch(`/api/interactions/share/${item._id}`, {
        method: 'POST',
      });
    } catch (e) {
      console.log(e);
      ...
      <View style={styles.cardHeader}>
        <View style={styles.sourceInfo}>
          <View style={styles.sourceLogo}>
            <Text style={styles.sourceLogoText}>{init}</Text>
          </View>
          <View>
            <View style={styles.sourceNameRow}>
              <Text
                style={[styles.sourceName, { color: isDark ? '#FFF' : '#111' }]}
              >
                {src}
              </Text>
              <CheckCircle2
        ...
        >
          <MoreHorizontal size={18} color={isDark ? '#FFF' : '#999'} />
        </TouchableOpacity>
      </View>
      <Text
        style={[styles.cardHeadline, { color: isDark ? '#FFF' : '#111' }]}
        numberOfLines={3}
      >
        {item.headline}
      </Text>
      ...
      <View style={styles.actionRow}>
        <View style={styles.actionGroup}>
          ...
          >
            <Heart
              size={14}
              color={isLiked ? '#FF2D55' : isDark ? '#DDD' : '#666'}
              fill={isLiked ? '#FF2D55' : 'transparent'}
            />
            <Text
              style={[
                styles.actionText,
                { color: isLiked ? '#FF2D55' : isDark ? '#DDD' : '#555' },
              ]}
            >
              {likes}
            </Text>
          ...
          >
            <MessageCircle size={14} color={isDark ? '#DDD' : '#666'} />
            <Text
              style={[styles.actionText, { color: isDark ? '#DDD' : '#555' }]}
            >
              {commentsCount}
            </Text>
          </TouchableOpacity>
          ...
          >
            <Eye size={14} color={isDark ? '#DDD' : '#666'} />
            <Text
              style={[styles.actionText, { color: isDark ? '#DDD' : '#555' }]}
            >
              {item.views || 0}
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          ...
          >
            <Bookmark
              size={14}
              color={isSaved ? '#3B82F6' : isDark ? '#DDD' : '#666'}
              fill={isSaved ? '#3B82F6' : 'transparent'}
            />
            <Text
              style={[
                styles.actionText,
                { color: isSaved ? '#3B82F6' : isDark ? '#DDD' : '#555' },
              ]}
            >
              {saves}
            </Text>
          ...
          >
            <Share size={14} color={isDark ? '#DDD' : '#666'} />
          </TouchableOpacity>
        </View>
      </View>
      {randomComment && (
        ...
        >
          <Text
            style={[styles.snippetUser, { color: isDark ? '#CCC' : '#555' }]}
            numberOfLines={1}
          >
            <Text
              style={{ fontWeight: '600', color: isDark ? '#FFF' : '#222' }}
            >
              {randomComment.user?.name || 'User'}{' '}
            </Text>
            {randomComment.text}
          </Text>
        </TouchableOpacity>
...
const WorldCard = ({ item, onPress }) => (
  ...
  >
    {item.image_link ? (
      <Image source={{ uri: item.image_link }} style={styles.worldImage} />
    ) : (
      <View style={[styles.worldImage, { backgroundColor: '#CCC' }]} />
    )}
    <LinearGradient
      colors={['transparent', 'rgba(0,0,0,0.88)']}
  ...
  if (!stack.length)
    return (
      <View style={styles.stackEmpty}>
        <Text style={{ color: '#999' }}>No more stories</Text>
      </View>
    );
          ...
          >
            ...
            >
              {item.image_link ? (
                ...
                <View
                  style={[styles.stackImage, { backgroundColor: '#DDD' }]}
                />
              )}
              <LinearGradient
              ...
              <View style={styles.stackContent}>
                ...
                <Text style={styles.stackHeadline} numberOfLines={3}>
                  {item.headline}
                </Text>
...
const SectionHeader = ({
  ...
  <View style={styles.sectionHead}>
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
      {Icon && (
        <Icon size={18} color={color === '#111' && isDark ? '#FFF' : color} />
      )}
      <Text
        style={[
          styles.sectionTitle,
          { color: color === '#111' && isDark ? '#FFF' : color },
        ]}
      >
        {title}
      </Text>
    ...
    >
      ...
      >
        <View style={styles.header}>
          <Text style={styles.headerLogo}>newsontip.</Text>
          <View style={styles.headerRight}>
            ...
            >
              <Bookmark size={20} color={isDark ? '#FFF' : '#111'} />
            </TouchableOpacity>
            <TouchableOpacity
              style={[
                styles.headerIcon,
            ...
            >
              <Bell size={20} color={isDark ? '#FFF' : '#111'} />
            </TouchableOpacity>
          </View>
        </View>
        ...
        >
          {trending.map((item, i) => (
            <TrendingCard
              ...
              item={item}
              onPress={updatedItem =>
                navigation.navigate('ArticleDetail', { article: updatedItem })
              }
            />
          ))}
        </ScrollView>
        ...
        {popular.slice(0, 5).map((item, i) => (
          <React.Fragment key={item._id || i}>
            <ArticleCard
              ...
              onOpenComments={() => setActiveCommentArticle(item._id)}
              navigation={navigation}
              isDark={isDark}
            />
            {i === 2 && <BannerAdComponent />}
          </React.Fragment>
        ))}
        ...
        {popular.slice(5).map((item, i) => (
          <React.Fragment key={'p2' + (item._id || i)}>
            <ArticleCard
              item={item}
              onPress={updatedItem =>
                navigation.navigate('ArticleDetail', { article: updatedItem })
      ...
      <CommentsSheet
        visible={!!activeCommentArticle}
        onClose={() => setActiveCommentArticle(null)}
        articleId={activeCommentArticle}
      />
    </View>
  );
};
