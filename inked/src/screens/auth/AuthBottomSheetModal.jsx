...
import { X, Check, Sparkles, ShieldCheck, Layers } from 'lucide-react-native';
...
const AuthBottomSheetModal = ({ navigation, route }) => {
  ...
  const { loginWithGoogle, updateTopics, user, toggleLike, toggleBookmark } =
    useAuth();
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderRelease: (_, gestureState) => {
        if (gestureState.dy > height * 0.15 || gestureState.vy > 0.5) {
          handleClose();
        } else {
          Animated.spring(slideAnim, {
            toValue: 0,
  ...
  const handleSaveTopics = async () => {
    ...
    if (route.params?.pendingAction) {
      const { type, article } = route.params.pendingAction;
      if (type === 'like') {
        toggleLike(article);
      } else if (type === 'save') {
    ...
    <View style={styles.modalOverlay}>
      ...
      >
        ...
        <View style={styles.headerRow}>
          <Text
            style={[styles.sheetTitle, { color: isDark ? '#FFF' : '#111' }]}
          >
            {view === 'auth' ? 'Welcome to NewsOnTip' : 'Personalize Your Wire'}
          </Text>
          <TouchableOpacity
            style={[
              styles.closeBtn,
              { backgroundColor: isDark ? '#262628' : '#F3F4F6' },
            ]}
            onPress={handleClose}
        ...
        {view === 'auth' && (
          <View style={{ paddingTop: 4 }}>
            ...
            >
              Sign in with your Google account to sync your saved bookmarks,
              tune your news categories, and unlock creator features.
            </Text>
            ...
            <View style={styles.perksContainer}>
              <View style={styles.perkRow}>
                <View
                  style={[
                    styles.perkIconBox,
                    { backgroundColor: isDark ? '#261E1E' : '#FEF2F2' },
                  ]}
                >
                  <Sparkles size={14} color="#DC2626" />
                </View>
                <Text
                  style={[styles.perkText, { color: isDark ? '#DDD' : '#333' }]}
                >
