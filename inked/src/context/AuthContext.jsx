...
export const AuthProvider = ({ children }) => {
    ...
    try {
      ...
      if (email) {
        ...
        showLocalNotification(
          ...
          "You're signed in to Inked. Stay ahead with today's top stories."
        );
    ...
    if (user && user.id) {
      try {
        await fetch(`${MAIN_BACKEND_URL}/api/user/bookmarks`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: user.id, article }),
        });
  ...
  // Toggle Like for an article headline
  const toggleLike = async (headline) => {
    if (!headline) return false;

    const exists = likedArticles.includes(headline);
    const updated = exists
      ? likedArticles.filter((h) => h !== headline)
    ...
    if (user && user.id) {
      try {
        await fetch(`${MAIN_BACKEND_URL}/api/user/likes`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: user.id, articleHeadline: headline }),
        });
  ...
  const isLiked = useCallback(
    (headline) => {
      if (!headline) return false;
      return likedArticles.includes(headline);
    },
    [likedArticles]
  );
