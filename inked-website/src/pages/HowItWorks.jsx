      ...
      <div style={{ borderBottom: '1px solid var(--rule-gray)' }}>
        <div className="container" style={{ padding: '3rem 1.5rem' }}>
          ...
          <h1 className="font-serif" style={{ fontSize: 'clamp(2rem, 5vw, 3.2rem)', fontWeight: 900, color: 'var(--text)', lineHeight: 1.15 }}>
            Why Inked exists.<br />How it runs.
          </h1>
        </div>
      </div>
      ...
      <div className="container" style={{ padding: '4rem 1.5rem', maxWidth: '760px' }}>
        ...
        <Section>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem', color: 'var(--caption-gray)', fontSize: '1rem', lineHeight: 1.75 }}>
            ...
            <p>Inked was built on a different assumption: the people who report, write, and analyze news are the product. Not the aggregation engine. A platform that captures attention from journalism but doesn't share value with journalists is extracting, not building.</p>
            <p>The mechanics of Inked — source attribution on every aggregated story, an identity-verified writing platform, a transparent engagement-weighted earnings pool — are all responses to that one premise. It's not a feature list. It's a position.</p>
        ...
        <Section>
          ...
          <h2 className="font-serif" style={{ fontSize: '1.8rem', fontWeight: 700, marginBottom: '1rem', color: 'var(--text)' }}>
            How news gets into Inked.
          </h2>
          <p style={{ color: 'var(--caption-gray)', fontSize: '1rem', lineHeight: 1.75, marginBottom: '1.5rem' }}>
            In plain language, not a tech spec:
          </p>
          {[
            { step: 'Pull', body: 'Inked connects to 20+ public RSS feeds and licensed APIs from Indian and international publishers — Times of India, The Hindu, Reuters, BBC, TechCrunch, and more. These are public distribution channels that publishers deliberately maintain for exactly this purpose.' },
            { step: 'Attribute', body: 'Every story that enters Inked carries the original publisher\'s name, the author\'s name where available, the publication date, and a direct link to the original article. We don\'t strip attribution. It\'s the first thing shown on every story.' },
            { step: 'Display', body: 'Stories are shown in the app as summaries with the source clearly labeled. Readers can read the summary or tap through to the full article at the original publication. Inked doesn\'t try to keep you on Inked instead of the publisher.' },
            { step: 'Separate', body: 'Original writing from Inked creators is clearly labeled as such — it never appears as if it\'s from an external publication, and aggregated content never appears as if it was written by an Inked creator.' },
          ].map((item, i) => (
        ...
        <Section>
          ...
          <p style={{ color: 'var(--caption-gray)', fontSize: '1rem', lineHeight: 1.75 }}>
            For <strong style={{ color: 'var(--text)' }}>original writing on Inked</strong>: every piece is reviewed by a moderator before publication. We check that factual claims are sourced, that no content violates our content policy, and that the piece is genuinely original. This review typically completes within 24 hours.
          </p>
        </Section>
